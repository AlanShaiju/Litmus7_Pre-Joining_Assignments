 ...
 *                       'Product".
 * @param controller     : Variable used to communicate with the controller
 *                       class.
...
public class ProductApp {
	...
	public static void main(String[] args) {
		...
		RetailController controller = new RetailController();
		int flag = 0;
		while (flag != 6) {
			...
			case 1:
				...
				scanner.nextLine();
				serverResponse = controller.addProduct(productId, name, category, price, stockQuantity);
			...
			case 2:
				...
				productId = scanner.nextInt();
				scanner.nextLine();
				serverResponse = controller.getProductById(productId);
				// Checks if the operation is a success.
				if (serverResponse.getStatusCode() == SUCCESS_CODE) {
				...
				break;
			case 3:
				serverResponse = controller.getAllProducts();
				// Checks if the operation is a success.
				if (serverResponse.getStatusCode() == SUCCESS_CODE) {
			...
			case 4:
				...
				if (serverResponse.getStatusCode() == SUCCESS_CODE) {
					...
					stockQuantity = scanner.nextInt();
					serverResponse = controller.updateProduct(productId, name, category, price, stockQuantity);
					// Checks if the operation is a success.
					if (serverResponse.getStatusCode() == SUCCESS_CODE) {
						System.out.println(serverResponse.getMessage());
					} else {
			...
			case 5:
				...
				// Checks if the operation is a success.
				if (serverResponse.getStatusCode() == SUCCESS_CODE) {
					serverResponse = controller.deleteProduct(productId);
					// Checks if the operation is a success.
					if (serverResponse.getStatusCode() == SUCCESS_CODE) {
						System.out.println(serverResponse.getMessage());
					} else {
